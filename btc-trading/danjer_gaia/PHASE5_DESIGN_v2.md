...
## 10. Round 30-37 反映: 取引所再選定 v7 確定

Bybit日本撤退判明 → 30+候補網羅評価 → **戦略Z 改訂版 v7** 確定 (Shuji決定 2026-06-04)。

### Phase 5+ Cap 3 ($100k) — Lighter 新興DEX paper先行検証

```
Hyperliquid 50% + Exness 50% 並走 (CB月$371)
└─ 新規追加: Lighter (zkSync L2 perp DEX) paper検証 12ヶ月
    ├─ TVL推移監視 ($487M → $1B 目標)
    ├─ 板厚成長確認 (BTC perp $30M → $100M)
    ├─ 公式Python SDK 整備状況確認
    └─ LIT token保有要件のコスト試算
```

### Phase 5+ Cap 5 ($500k) — Lighter Live並走候補

```
条件達成チェック:
- Lighter TVL $1B到達
- 歴史2年経過 (mainnet 2025年10月 → 2027年10月クリア)
- Hyperliquid Cap 3 障害履歴 ゼロ件
- LIT token保有コスト < CB効果

条件達成時の構成:
Hyperliquid 40% + Exness 40% + Lighter 20%
└─ 3系統並走 (CB総額月$1,500-2,000)
```

### FXGT (不採用、 Shuji決定 Round 37)

GPT/Gemini が Round 36 で提案した **FXGT Optimus口座ボーナス15,000円** は、 Shuji判断で **不採用** 確定。

理由: 管理アカウント増加+税務処理複雑化+1.5万円のために KYC手続きの時間対価値が低い。 「シンプルが正義」 のShuji判断哲学。

### v7 全Phase 統合俯瞰

```
[Phase 1] 評価基盤 (Day 1-14) ✅完了
        ↓
[Phase 2 v7] Stage 0/1 (Day 22-52) — Hyperliquid 2x、 $15-50
        ↓
[Phase 3 v7] BTC再現性検証 (Day 53-167) — Hyperliquid 3x、 $50-2,250
    Phase 3後半: MT5/Exness ZMQ ブリッジ実装 (Round 34、 10-16日工数)
        ↓
[Phase 4 v7] Cap 1→Cap 2 (Day 168-247) — Hyperliquid + Exness 並走
    Cap 1 ($25k): Hyperliquid 60% + Exness 40% (CB月$93)
    Cap 2 ($50k): 50:50 (CB月$185)
        ↓
[Phase 5+ v7] 守りの永続 (Day 247+、 永続)
    Cap 3 ($100k):  Lighter paper 12ヶ月検証開始
    Cap 4 ($250k):  Lighter 条件達成確認 (TVL $1B + 歴史2年)
    Cap 5 ($500k):  Lighter Live並走 (条件達成時、 3系統)
        └─ 待機: bitget / GMOコイン / bitFlyer Lightning
```

### v7 R88 リスク追加 (Lighter 検証)

- **R88 (Lighter 新興DEX 信頼性)**: TVL $487M、 歴史8ヶ月 (2026-06時点) → Phase 5+ Cap 3 から **12ヶ月paper先行**、 Cap 5 で TVL $1B + 歴史2年 + LIT保有コスト合理性 すべてクリア時のみ Live並走
