...
| R44 | ETH/SOL データ品質 | 3ソース突合 (Hyperliquid+bitget+Coinglass、 Round 30-33 でBybit→Hyperliquid置換) |
...
Claudeとしては **Phase 3 v2 はShujiさん指示完全準拠** なので、 Round 26 は省略可。 直接 Phase 4 設計や、 Embedding完了待ち+Hyperliquid testnet 着手に進める (Round 30-33 でBybit→Hyperliquid置換)。

## 11. Round 30-33 反映 (v3 取引所変更、 2026-06-04)

主軸取引所が Bybit → **Hyperliquid (DEX)** に変更されたことに伴い、 Phase 3 設計に以下を反映:

- Stage 2 3xレバ運用 → **Hyperliquid 50xのうち 3x のみ使用** (R66 流動性影響対応)
- exchange/bybit_client.py → **exchange/hyperliquid_client.py** (公式Python SDK経由)
- 副取引所: **bitget** (Phase 3後半 Day 100前後 から並走テスト、 障害退避準備)
- 取引手数料 (Bybit前提): $5-20 → **Hyperliquid前提**: $2-15 (maker rebate効果)
- 緊急退避テスト追加 (Day 100前後): **Hyperliquid出金 24-48h遅延の実機検証** (R79対応)
- Wallet階層 維持: Ledger Cold $2k-10k + MetaMask Hot $500-5k
- R44 (ETH/SOLデータ品質): 3ソース突合先を Hyperliquid+bitget+Coinglass に更新

Phase 3 月額コスト見直し: $71-170 → **$50-120** (-$20-50節約、 maker rebate効果)
