...
配達完了画面の場合(summaryは「店舗名 ドロップ先町名 HH:MM ¥金額 時間 距離 件数」の順。HH:MMはスクショ画面の1番上に表示されている時刻を24時間形式で。町名は市区より後だけ。件数=画面に表示されているポイント数。「2ポイント」なら「2件」):
...
注文詳細の場合(summaryは「店舗名 ドロップ先町名 HH:MM ¥金額 時間 距離 件数」の順。HH:MMはスクショの1番上の時刻):
