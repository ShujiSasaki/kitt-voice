#!/bin/bash
# R67反映: self-test → uvicorn + relay_worker 再起動 (routing_validation + Watchdog部屋単位化)
set -x
...
nohup python3 -m uvicorn meeting_system.server:create_app --factory \
  ...
  --ssl-certfile meeting_system/certs/cert.pem > /tmp/uvicorn_r67.log 2>&1 &

nohup caffeinate -dimsu python3 scripts/relay_worker.py > /tmp/relay_worker_router.log 2>&1 &
sleep 6

echo "=== uvicorn ==="
tail -3 /tmp/uvicorn_r67.log
echo "=== worker ==="
